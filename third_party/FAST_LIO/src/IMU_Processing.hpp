...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      RCLCPP_INFO(rclcpp::get_logger("laserMapping"), "IMU Initial Done");
      // RCLCPP_INFO(rclcpp::get_logger("laserMapping"), "IMU Initial Done: Gravity: %.4f %.4f %.4f %.4f; state.bias_g: %.4f %.4f %.4f; acc covarience: %.8f %.8f %.8f; gry covarience: %.8f %.8f %.8f",\
